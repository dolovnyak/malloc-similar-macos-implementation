...
TEST(Take_Mem_From_Free_Nodes, Main_Check) {
...

    {
        ...
        t_memory_node* node = (t_memory_node*)(CAST_TO_BYTE_APPLY_ZONE_SHIFT(test_zone) + 38 + 26 + sizeof(t_memory_node) * 2);
        ASSERT_EQ(CAST_TO_BYTE_APPLY_NODE_SHIFT(node), (BYTE*)mem);
...

    {
        ...
        t_memory_node* new_separated_node = (t_memory_node*)(CAST_TO_BYTE_APPLY_ZONE_SHIFT(test_zone) + 48 + sizeof(t_memory_node));
...

    {
        ...
        t_memory_node* new_separated_node = (t_memory_node*)(CAST_TO_BYTE_APPLY_ZONE_SHIFT(test_zone) + 47 + 48 + sizeof(t_memory_node) * 2);
...

    {
        ...
        t_memory_node* new_separated_node = (t_memory_node*)(CAST_TO_BYTE_APPLY_ZONE_SHIFT(test_zone) + 47 + 22 + 48 + sizeof(t_memory_node) * 3);
...
TEST(Take_Memory_From_Zone, Main_Check) {
...

    {
        ...
        t_zone* zone = (t_zone*)test_zone;
        size_t start_zone_size = TEST_ZONE_SIZE - sizeof(t_zone) - SIZE_WITH_NODE_HEADER(47) - SIZE_WITH_NODE_HEADER(22);
...
/// написать тест, что тип алокации корректно выбирается и новые зоны корректно создаются/удаляются
